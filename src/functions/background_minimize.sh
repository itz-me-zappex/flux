...
background_minimize(){
  ...
  if ! "${PREFIX}/lib/flux/window-minimize" "$passed_window_id" > /dev/null 2>&1; then
    message --warning "Unable to minimize window with XID $passed_window_id of process '$passed_process_name' with PID $passed_process_pid on unfocus event!"
  else
    message --info "Window with XID $passed_window_id of process '$passed_process_name' with PID $passed_process_pid has been minimized on unfocus event."
  fi
}
