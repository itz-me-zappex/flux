...
cursor_ungrab(){
  local local_background_focus_cursor_grab_pid="${background_focus_cursor_grab_map["$passed_window_xid"]}"

  if check_pid_existence "$local_background_focus_cursor_grab_pid"; then
    if ! kill "$local_background_focus_cursor_grab_pid" > /dev/null 2>&1; then
