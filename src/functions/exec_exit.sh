...
exec_exit(){
  ...
  if [[ "$passed_focused_section" == "$passed_section" &&
    ...
    while read -r local_temp_command ||
      ...
      passed_section="$passed_section" \
      passed_event_command="$local_temp_command" \
      passed_end_of_msg="$passed_end_of_msg because window with XID $passed_window_xid of process '$passed_process_name' with PID $passed_process_pid appears focused at this moment" \
  ...
  if [[ "$passed_focused_section" != "$passed_section" && 
    ...
    while read -r local_temp_command ||
      ...
      passed_section="$passed_section" \
      passed_event_command="$local_temp_command" \
      passed_end_of_msg="$passed_end_of_msg because window with XID $passed_window_xid of process '$passed_process_name' with PID $passed_process_pid appears unfocused at this moment" \
