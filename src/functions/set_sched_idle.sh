...
set_sched_idle(){
  ...
  if [[ -z "$local_chrt_error" ]]; then
    ...
    if [[ -z "$sched_realtime_is_supported" &&
      ...
      message --warning "Daemon has insufficient rights to restore realtime scheduling policy for process '$local_process_name' with PID $local_pid, changing it to 'idle' on window $local_temp_window_xid unfocus event has been cancelled!"
      local local_idle_cancelled='1'
    elif (( UID != 0 )) &&
         [[ "${sched_previous_policy_map["$local_pid"]}" == 'SCHED_DEADLINE' ]]; then
      message --warning "Daemon has insufficient rights to restore deadline scheduling policy for process '$local_process_name' with PID $local_pid, changing it to 'idle' on window $local_temp_window_xid unfocus event has been cancelled!"
      local local_idle_cancelled='1'
      ...
      message --warning "Process '$local_process_name' with PID $local_pid already has scheduling policy set to 'idle', changing it on window $local_temp_window_xid unfocus event has been cancelled!"
      local local_idle_cancelled='1'
    fi
    ...
    message --warning "Unable to obtain scheduling policy info of process '$local_process_name' with PID $local_pid, changing it to 'idle' on window $local_temp_window_xid unfocus event has been cancelled!"
  fi
}
