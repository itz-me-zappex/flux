...
background_fps_limit(){
  ...
  if [[ "$local_unfocus_limits_delay" != '0' ]]; then
    ...
    message --verbose "MangoHud config file ($local_shorten_path_result) ($passed_section) will be FPS limited after $local_unfocus_limits_delay second(s) on window ($passed_window_xid) unfocus event of process '$passed_process_name' ($passed_pid)."
    sleep "$local_unfocus_limits_delay"
  fi
  
  if check_pid_existence "$passed_pid"; then
    if mangohud_fps_set "$passed_section" "$local_mangohud_config" "$local_mangohud_source_config" "$local_fps_unfocus"; then
      if [[ "$local_unfocus_limits_delay" == '0' ]]; then
        local local_shorten_path_result
        shorten_path "$local_mangohud_config"
        ...
        local local_shorten_path_result
        shorten_path "$local_mangohud_config"
        message --info "MangoHud config file ($local_shorten_path_result) ($passed_section) limited to $local_fps_unfocus FPS after $local_unfocus_limits_delay second(s) on window ($passed_window_xid) unfocus event of process '$passed_process_name' ($passed_pid)."
      fi
    fi
  else
