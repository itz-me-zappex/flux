...
background_sched_idle(){
  ...
  if [[ "$local_limits_delay" != '0' ]]; then
    message --verbose "Scheduling policy of process '$passed_process_name' with PID $passed_pid will be changed to 'idle' after $local_limits_delay second(s) on window $passed_window_xid unfocus event."
  ...
  if check_pid_existence "$passed_pid"; then
    if ! chrt --idle --pid 0 "$passed_pid" > /dev/null 2>&1; then
      ...
      if [[ "$local_limits_delay" == '0' ]]; then
        ...
        message --info "Scheduling policy of process '$passed_process_name' with PID $passed_pid has been changed to 'idle' after $local_limits_delay second(s) on window $passed_window_xid unfocus event."
      fi
    fi
  else
