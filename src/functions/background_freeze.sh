...
background_freeze(){
  ...
  if [[ "$local_limits_delay" != '0' ]]; then
    message --verbose "Process '$passed_process_name' with PID $passed_pid will be frozen after $local_limits_delay second(s) on window $passed_window_xid unfocus event."
  ...
  if check_pid_existence "$passed_pid"; then
    if ! kill -STOP "$passed_pid" > /dev/null 2>&1; then
      ...
      if [[ "$local_limits_delay" == '0' ]]; then
        ...
        message --info "Process '$passed_process_name' with PID $passed_pid has been frozen after $local_limits_delay second(s) on window $passed_window_xid unfocus event."
      fi
    fi
  else
