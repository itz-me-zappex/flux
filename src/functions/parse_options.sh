    ...
    --get | -g | --get=* )
      ...
      if (( select_window_exit_code > 0 )) ; then
        ...
        if (( get_process_info_exit_code == 1 )); then
          message --error "Unable to obtain info about process with PID $pid of window $window_xid! Probably process has been terminated during check."
          exit 1
        elif (( get_process_info_exit_code == 2 )); then
          message --error "Unable to obtain owner username of process $process_name with PID $pid of window $window_xid!"
          exit 1
        fi
