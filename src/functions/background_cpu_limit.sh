...
background_cpu_limit(){
  ...
  if [[ "$local_limits_delay" != '0' ]]; then
    ...
    trap 'message --info "Delayed for $local_limits_delay second(s) CPU limiting of process '"'$passed_process_name'"' ($passed_pid) of window ($passed_window_xid) cancelled because of daemon termination." ; \
    kill "$local_sleep_pid"; \
    exit 0' SIGINT SIGTERM

    trap 'message --info "Delayed for $local_limits_delay second(s) CPU limiting of process '"'$passed_process_name'"' ($passed_pid) cancelled on window ($passed_window_xid) focus event." ; \
    kill "$local_sleep_pid"; \
    exit 0' SIGUSR1

    trap 'message --info "Delayed for $local_limits_delay second(s) CPU limiting of process '"'$passed_process_name'"' ($passed_pid) cancelled on window ($passed_window_xid) closure event." ; \
    kill "$local_sleep_pid"; \
    exit 0' SIGUSR2
  ...
  if check_pid_existence "$passed_pid"; then
    if [[ "$local_limits_delay" == '0' ]]; then
      ...
      message --info "Process '$passed_process_name' ($passed_pid) CPU limited to $local_cpu_limit% after $local_limits_delay second(s) on window ($passed_window_xid) unfocus event."
    fi
    ...
    if [[ -n "$sched_realtime_is_supported" ]]; then
      if ! chrt --fifo --pid 99 "$local_cpulimit_pid" > /dev/null 2>&1; then
        ...
        message --info "Scheduling policy of 'cpulimit' ($local_cpulimit_pid) hooked to process '$passed_process_name' ($passed_pid) of window ($passed_window_xid) changed to 'FIFO'."
      fi
    fi
