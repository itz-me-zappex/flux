...
unfreeze_process(){
  ...
  if [[ "$local_unfocus_limits_delay" != '0' ]] &&
    ...
    if ! kill "$local_background_freeze_pid" > /dev/null 2>&1; then
      message --warning "Unable to cancel delayed for $local_unfocus_limits_delay second(s) freezing of process '$passed_process_name' ($passed_pid) $passed_end_of_msg!"
    else
      message --info "Delayed for $local_unfocus_limits_delay second(s) freezing of process $passed_process_name' ($passed_pid) cancelled $passed_end_of_msg."
    fi
  else
