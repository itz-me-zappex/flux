...
handle_closure(){
  ...
  for local_temp_existing_window in $local_existing_windows; do
    local local_existing_window_ids_array+=("${local_temp_existing_window/'='*/}")
  done
  ...
  for local_temp_window_id in "${!cache_process_pid_map[@]}"; do
    ...
    if [[ " ${local_existing_window_ids_array[*]} " != *" $local_temp_window_id "* ]]; then
      local local_terminated_window_ids_array+=("$local_temp_window_id")
    fi
  done
  ...
  for local_temp_terminated_window_id in "${local_terminated_window_ids_array[@]}"; do
    ...
    if [[ -n "${cache_process_pid_map["$local_temp_terminated_window_id"]}" ]]; then
      ...
      local local_terminated_process_pid="${cache_process_pid_map["$local_temp_terminated_window_id"]}"
      local local_terminated_section="${cache_section_map["$local_terminated_process_pid"]}"
      local local_terminated_process_name="${cache_process_name_map["$local_temp_terminated_window_id"]}"
      local local_terminated_process_owner="${cache_process_owner_map["$local_temp_terminated_window_id"]}"
      local local_terminated_process_owner_username="${cache_process_owner_username_map["$local_temp_terminated_window_id"]}"
      ...
      if [[ -n "${is_freeze_applied_map["$local_terminated_process_pid"]}" ]]; then # Unfreeze process if frozen
        ...
        for local_temp_existing_window_id in "${local_existing_window_ids_array[@]}"; do
          ...
          local local_existing_process_pid="${cache_process_pid_map["$local_temp_existing_window_id"]}"
