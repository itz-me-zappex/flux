# To unset limits for terminated windows and remove info
# about them from cache
handle_closure(){
  ...
  for local_temp_window_xid in "${!cache_pid_map[@]}"; do
    # Add window XID to array with terminated windows if it does not
    # exist in '_NET_CLIENT_LIST_STACKING'
  ...
  for local_temp_terminated_window_xid in "${local_terminated_window_xids_array[@]}"; do
    ...
    if [[ -n "${cache_pid_map["$local_temp_terminated_window_xid"]}" ]]; then
      ...
      if [[ -n "${background_freeze_pid_map["$local_terminated_pid"]}" ]]; then
        ...
        # Do not remove FPS limit if one of existing windows matches this section
      ...
      # Restore scheduling policy if changed
      ...
      for local_temp_cached_pid in "${cache_pid_map[@]}"; do
        # Mark to avoid unset of matching section if that is not
        # last window of process
      ...
      # Remove matching section associated with PID if there is no
      # other window processes with the same PID
      if [[ -z "$local_found" ]]; then
