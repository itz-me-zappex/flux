  ...
  for local_temp_terminated_window_xid in "${local_terminated_window_xids_array[@]}"; do
    ...
    if [[ -n "${cache_process_pid_map["$local_temp_terminated_window_xid"]}" ]]; then
      ...
      if [[ -n "${request_freeze_map["$local_terminated_process_pid"]}" ]]; then
        ...
        message --verbose "MangoHud config file '$(shorten_path "${config_key_mangohud_config_map["$local_terminated_section"]}")' FPS limiting from section '$local_terminated_section' has been cancelled due to window with XID $local_temp_terminated_window_xid closure of process '$local_terminated_process_name' with PID $local_terminated_process_pid."
