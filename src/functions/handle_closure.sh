...
handle_closure(){
  ...
  for local_temp_terminated_window_xid in "${local_terminated_window_xids_array[@]}"; do
    ...
    if [[ -n "${cache_process_pid_map["$local_temp_terminated_window_xid"]}" ]]; then
      ...
      local local_end_of_msg="due to closure of window with XID $local_temp_window_xid"
      ...
      if [[ -n "${background_freeze_pid_map["$local_terminated_process_pid"]}" ]]; then
        ...
        if [[ -z "$local_found" ]]; then
          passed_section="$local_terminated_section" \
          passed_end_of_msg="due to closure of window with XID $local_temp_window_xid of process '$local_terminated_process_name' with PID $local_terminated_process_pid" \
          unset_fps_limit
        else
          unset local_found
        fi
      ...
      if [[ -n "${request_freeze_map["$local_terminated_process_pid"]}" ]]; then
        ...
        message --verbose "Freezing of process '$local_terminated_process_name' with PID $local_terminated_process_pid has been cancelled due to closure of window with XID $local_temp_terminated_window_xid."
      elif [[ -n "${request_cpu_limit_map["$local_terminated_process_pid"]}" ]]; then
        unset request_cpu_limit_map["$local_terminated_process_pid"]
        message --verbose "CPU limiting of process '$local_terminated_process_name' with PID $local_terminated_process_pid has been cancelled due to closure of window with XID $local_temp_terminated_window_xid."
      elif [[ -n "$local_terminated_section" &&
      ...
      if [[ -n "${request_sched_idle_map["$local_terminated_process_pid"]}" ]]; then
        ...
        message --verbose "Changing scheduling policy to 'idle' for process '$local_terminated_process_name' with PID $local_terminated_process_pid has been cancelled due to closure of window with XID $local_temp_terminated_window_xid."
      fi
