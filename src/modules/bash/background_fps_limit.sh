...
background_fps_limit(){
	...
	if [[ "${config_key_delay_map["$passed_section"]}" != '0' ]]; then
		message --verbose "MangoHud config file '${config_key_mangohud_config_map["$passed_section"]}' from section '$passed_section' will be FPS limited after ${config_key_delay_map["$passed_section"]} second(s) due to window $passed_window_id unfocus event."
