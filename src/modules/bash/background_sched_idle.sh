...
background_sched_idle(){
	...
	if check_pid_existence "$passed_process_pid"; then
		...
		if ! chrt --idle --pid 0 "$passed_process_pid"; then
			message --warning "Scheduling policy of process '$passed_process_name' with PID $passed_process_pid cannot be changed to 'idle' on unfocus event!"
		else
			message --info "Scheduling policy of process '$passed_process_name' with PID $passed_process_pid has been changed to 'idle' on unfocus event."
		fi
	else
