...
unset_terminated_limits(){
	...
	for local_temp_terminated_window_id in $local_terminated_windows_ids; do
	...
	for local_temp_terminated_window_id in $local_terminated_windows_ids; do
		...
		if [[ "${cache_event_type_map["$local_temp_terminated_window_id"]}" == 'bad' ]]; then
			...
			if [[ -n "${request_freeze_map["$local_terminated_process_pid"]}" ]]; then
				...
				message --info "Freezing of process '${cache_process_name_map["$local_temp_terminated_window_id"]}' with PID $local_terminated_process_pid has been cancelled due to window termination."
			elif [[ -n "${request_cpu_limit_map["$local_terminated_process_pid"]}" ]]; then
				request_cpu_limit_map["$local_terminated_process_pid"]=''
				message --info "CPU limiting of process '${cache_process_name_map["$local_temp_terminated_window_id"]}' with PID $local_terminated_process_pid has been cancelled due to window termination."
...
handle_terminated_windows(){
	...
	local local_terminated_windows_ids="${event/'terminated: '/}" # Remove everything before including type name of list with windows IDs
	local local_terminated_windows_ids="${local_terminated_windows_ids/'; existing: '*/}" # Remove list of existing windows IDs
	# Obtain list of existing windows IDs
	local local_existing_windows_ids="${event/*'existing: '/}" # Remove everything including type name of list with windows IDs
