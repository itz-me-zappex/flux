# To handle color mode with prefixes and timestamp specified by user
configure_prefixes(){
  # Define whether daemon should disable or enforce colors
  # for default prefixes and timestamp
  ...
  # Set specified timestamp format if any and handle
  ...
  # Those values will be set to main prefixes and timestamp if 'never'
  # is enforced
  if [[ "$color" != 'always' &&
  ...
  # Define whether daemon should enforce colorless or colorful
  # prefixes and timestamp
  if [[ "$color" == 'never' ]]; then
