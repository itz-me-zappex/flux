...
validate_sched(){
  ...
  if (( UID == 0 )); then
    ...
    if ! chrt --other --pid 0 "$local_sleep_pid" > /dev/null 2>&1; then
      message --warning "Daemon has insufficient rights to change scheduling policies!"
      message --warning "To make 'unfocus-sched-idle' config key work and apply optimizations to 'cpulimit' and 'flux-grab-cursor', add your user to 'flux' group and reboot!"
    else
      sched_change_is_supported='1'
    fi
