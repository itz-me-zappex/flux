...
| `name` | Name of process, required if `command` is not specified. Daemon uses process executable name as process name, because `/proc/<PID>/comm` is stripped to 15 visible symbols. You can safely use executable name of both native and Wine/Proton applications here. |
